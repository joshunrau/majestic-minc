  ...
  then
    ...
    if [[ ! -f ${EXTRACTION_INVERSE_WARP} ]];
      ...
      if [[ ! -f ${EXTRACTION_SEGMENTATION} ]];
        ...
        ## superstep 1b ##
        logCmd ${ANTSPATH}/ThresholdImage ${DIMENSION} ${EXTRACTION_MASK_PRIOR_WARPED} ${EXTRACTION_MASK_PRIOR_WARPED_TMP} 0.5 1 1 0
        logCmd mv ${EXTRACTION_MASK_PRIOR_WARPED_TMP} ${EXTRACTION_MASK_PRIOR_WARPED}
        logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${EXTRACTION_MASK} MD ${EXTRACTION_MASK_PRIOR_WARPED} 2
        logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${EXTRACTION_MASK_TMP} GetLargestComponent ${EXTRACTION_MASK}
        logCMD mv ${EXTRACTION_MASK_TMP} ${EXTRACTION_MASK}
       ...
       # Pad image here to avoid errors from dilating into the edge of the image
        ...
        logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${EXTRACTION_SEGMENTATION_TMP} PadImage ${EXTRACTION_SEGMENTATION} $padVoxels
        logCmd mv ${EXTRACTION_SEGMENTATION_TMP} ${EXTRACTION_SEGMENTATION}
        logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${EXTRACTION_MASK_PRIOR_WARPED_TMP} PadImage ${EXTRACTION_MASK_PRIOR_WARPED} $padVoxels
        logCmd mv ${EXTRACTION_MASK_PRIOR_WARPED_TMP} ${EXTRACTION_MASK_PRIOR_WARPED}
        ...
        logCmd ${ANTSPATH}/MultiplyImages ${DIMENSION} ${EXTRACTION_GM} ${EXTRACTION_TMP} ${EXTRACTION_GM_TMP}
        logCmd mv ${EXTRACTION_GM_TMP} ${EXTRACTION_GM}

        logCmd ${ANTSPATH}/MultiplyImages ${DIMENSION} ${EXTRACTION_WM} ${ATROPOS_WM_CLASS_LABEL} ${EXTRACTION_WM_TMP}
        logCmd mv ${EXTRACTION_WM_TMP} ${EXTRACTION_WM}
        logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${EXTRACTION_TMP} ME ${EXTRACTION_CSF} 10

        logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${EXTRACTION_GM_TMP} addtozero ${EXTRACTION_GM} ${EXTRACTION_TMP}
        logCmd mv ${EXTRACTION_GM_TMP} ${EXTRACTION_GM} 
        logCmd ${ANTSPATH}/MultiplyImages ${DIMENSION} ${EXTRACTION_GM} ${ATROPOS_GM_CLASS_LABEL} ${EXTRACTION_GM_TMP}
        logCmd mv ${EXTRACTION_GM_TMP} ${EXTRACTION_GM}
        ...
        logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${EXTRACTION_MASK_TMP} addtozero ${EXTRACTION_MASK} ${EXTRACTION_TMP}
        logCmd mv ${EXTRACTION_MASK_TMP} ${EXTRACTION_MASK} 
        logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${EXTRACTION_MASK_TMP} ME ${EXTRACTION_MASK} 2
        logCmd mv ${EXTRACTION_MASK_TMP} ${EXTRACTION_MASK} 
        logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${EXTRACTION_MASK_TMP} GetLargestComponent ${EXTRACTION_MASK}
        logCmd mv ${EXTRACTION_MASK_TMP} ${EXTRACTION_MASK} 
        logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${EXTRACTION_MASK_TMP} MD ${EXTRACTION_MASK} 4
        logCmd mv ${EXTRACTION_MASK_TMP} ${EXTRACTION_MASK} 
        logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${EXTRACTION_MASK_TMP} FillHoles ${EXTRACTION_MASK} 2
        logCmd mv ${EXTRACTION_MASK_TMP} ${EXTRACTION_MASK} 
        logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${EXTRACTION_MASK_TMP} addtozero ${EXTRACTION_MASK} ${EXTRACTION_MASK_PRIOR_WARPED}
        logCmd mv ${EXTRACTION_MASK_TMP} ${EXTRACTION_MASK} 
        logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${EXTRACTION_MASK_TMP} MD ${EXTRACTION_MASK} 5
        logCmd mv ${EXTRACTION_MASK_TMP} ${EXTRACTION_MASK} 
        logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${EXTRACTION_MASK_TMP} ME ${EXTRACTION_MASK} 5
        logCmd mv ${EXTRACTION_MASK_TMP} ${EXTRACTION_MASK} 
        ...
        for img in ${EXTRACTION_SEGMENTATION} ${EXTRACTION_MASK} ${EXTRACTION_WM} ${EXTRACTION_GM} ${EXTRACTION_CSF} ${EXTRACTION_MASK_PRIOR_WARPED}
          do
            logCmd ${ANTSPATH}/ImageMath ${DIMENSION} ${TEMPDIR}/unpad.${OUTPUT_PREFIX} PadImage ${img} -$padVoxels
            logCmd mv ${TEMPDIR}/unpad.${OUTPUT_PREFIX} ${img}
          done
